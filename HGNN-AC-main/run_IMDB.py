...
ap.add_argument('--cuda', default=False, help='Using GPU or not.')
args = ap.parse_args()
print(args)
...
for cur_repeat in range(repeat):
    ...
    for epoch in range(num_epochs):
        ...
        logits, _, _, loss_ac, contrast_loss = net(
            ...
            target_node_indices, epoch, 'train')

        logp = F.log_softmax(logits, 1)
        loss_classification = F.nll_loss(logp[train_idx], labels[train_idx])
        train_loss = loss_classification + loss_lambda*loss_ac + 0.01*contrast_loss
        ...
        with torch.no_grad():
            logits, _, _, loss_ac, contrast_loss = net(
                (adjM, features_list, emb, mask_list, feat_keep_idx, feat_drop_idx, src_node_type),
                (g_lists, type_mask, edge_metapath_indices_lists), target_node_indices, epoch, 'vali')
            logp = F.log_softmax(logits, 1)
            loss_classification = F.nll_loss(logp[val_idx], labels[val_idx])
            val_loss = loss_classification + loss_lambda*loss_ac + 0.01*contrast_loss
    ...
    with torch.no_grad():
        _, embeddings, _, _, _ = net(
            (adjM, features_list, emb, mask_list, feat_keep_idx, feat_drop_idx, src_node_type),
            (g_lists, type_mask, edge_metapath_indices_lists),
            target_node_indices, epoch, 'test')
